...
export default function OccurrenceDetailPage({ params }: PageProps) {
  ...
  if (loading) {
    return <div className="text-center py-12 text-gray-500">A carregar detalhes...</div>;
  }
    ...
    <div className="flex flex-col gap-6 max-w-3xl">
      <div>
        ...
        <div className="flex justify-between items-start gap-4 flex-wrap">
          <div>
            <span className="text-xs text-gray-500">ID: {occurrence.id}</span>
      ...
      <div className="p-6 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-sm flex flex-col gap-4">
        <div>
          <h3 className="font-semibold text-gray-900 dark:text-white text-sm">Descrição</h3>
          <p className="text-gray-600 dark:text-gray-400 mt-1 leading-relaxed text-sm">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <h3 className="font-semibold text-gray-900 dark:text-white text-sm">Categoria</h3>
            <p className="text-gray-650 mt-1 text-sm">{occurrence.category}</p>
          </div>
          <div>
            <h3 className="font-semibold text-gray-900 dark:text-white text-sm">Reportado em</h3>
            <p className="text-gray-650 mt-1 text-sm">{new Date(occurrence.createdAt).toLocaleString()}</p>
          </div>
        </div>

        <div>
          <h3 className="font-semibold text-gray-900 dark:text-white text-sm">Coordenadas</h3>
      ...
      <div className="p-6 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-sm">
        <h3 className="font-semibold text-gray-900 dark:text-white text-sm mb-4">Ações de Gestão</h3>
