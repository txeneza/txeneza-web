...
export const AppMockup: React.FC = () => {
  return (
    <div className="relative mx-auto w-full max-w-[280px] sm:max-w-[320px] md:max-w-[340px] aspect-[9/19.5]">
      ...
      <div className="hidden sm:flex absolute -right-6 top-[18%] items-center gap-2 bg-white dark:bg-grey900 border border-grey200 dark:border-grey800 rounded-md pl-2.5 pr-3.5 py-2 shadow-lg">
        <span className="w-2 h-2 shrink-0 rounded-full bg-forestGreen dark:bg-limeGreen" />
        <span className="font-mono text-[11px] font-medium text-forestGreen dark:text-grey50 whitespace-nowrap">
          Tempo real
        </span>
      </div>

      {/* Badge flutuante: pendentes (lateral esquerda) */}
      <div className="hidden md:flex absolute -left-8 top-[46%] flex-col gap-0.5 bg-white dark:bg-grey900 border border-grey200 dark:border-grey800 rounded-md px-4 py-2.5 shadow-lg">
        <div className="flex items-center gap-1.5 text-amber-600 dark:text-amber-400">
          <AlertTriangle className="w-3.5 h-3.5" />
          <span className="font-mono text-lg font-semibold leading-none">4</span>
        </div>
        <span className="text-[10px] font-semibold text-grey600 dark:text-grey400 uppercase tracking-wide">
          Pendentes
        </span>
      </div>

      {/* Badge flutuante: resolvidas (canto inferior direito) */}
      <div className="hidden sm:flex absolute -right-5 bottom-[16%] items-center gap-2 bg-white dark:bg-grey900 border border-grey200 dark:border-grey800 rounded-md pl-2.5 pr-3.5 py-2 shadow-lg">
